...
## Aligning columns
To align columns, first ensure you that you have published the assets. Then use Laravel's built-in [`$loop->index`](https://laravel.com/docs/blade#the-loop-variable) variable to append the correct style tag in `/resources/views/vendor/laravel-views/components/table.blade.php`. Here is an example where we are align currency that appears in column index 5 to the right:
